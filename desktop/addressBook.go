...
func adddressBookDia(pwd string) {
	...
	// Usage
	askPwdDia(core.UserSettings.AskPwd, pwd, mainWindowGui, func(correct bool) {
		...
		moveUp := func(index int) {
			if index > 0 {
				...
				if err := core.SaveAddressBook(core.UserAddressBook, pwd, ""); err != nil {
		...
		buildWalletButtons = func() {
			...
			for index, walletName := range core.UserAddressBook.WalletOrder {
				...
				renameButton := widget.NewButtonWithIcon("", theme.DocumentCreateIcon(), func() {
					...
					renameDia := dialog.NewCustomWithoutButtons(fmt.Sprintf("Rename %s", core.UserAddressBook.Wallets[walletName].Address), renameContent, mainWindowGui)
