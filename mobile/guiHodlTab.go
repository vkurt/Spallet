...
func hodlGui(creds core.Credentials) {
	hodlTabScroll := container.NewVScroll(container.NewVBox())
	hodlTabHeader := widget.NewLabelWithStyle("Hodling Details", fyne.TextAlignCenter, fyne.TextStyle{Bold: true})
	...
	// Claiming Kcal stuff
	remainedTimeForKcalGen := time.Unix(currentTime.Unix()+core.LatestAccountData.RemainedTimeForKcalGen, 0).Local()
	remainedTimeForKcalGenLabel := widget.NewLabel(fmt.Sprintf("Next Kcal generation time:\n\t%v", remainedTimeForKcalGen))
	remainedTimeForKcalGenLabel.Wrapping = fyne.TextWrapWord
	unclaimedBalanceLabel := widget.NewLabel(fmt.Sprintf("Earned Sparks: %s Kcal", core.FormatBalance(core.LatestAccountData.StakedBalances.Unclaimed, core.KcalDecimals)))
	...
	kcalClaimButton := widget.NewButton("Forge with Sparks", func() {
		...
		d := dialog.NewCustomWithoutButtons("Forge with Sparks", kcalConfirmDialog, mainWindow)
	...
	//  Staking stufff
	stakedBalancesLabel := widget.NewLabel(fmt.Sprintf("Specky's Soul stash:\n\t%s Soul", core.FormatBalance(core.LatestAccountData.StakedBalances.Amount, core.SoulDecimals)))
	...
	stakeButton := widget.NewButton("Power Up", func() {
		...
		d := dialog.NewCustomWithoutButtons("Power Up Specky", stakeConfirmDialog, mainWindow)
		d.Resize(mainWindowLyt.Selected().Content.Size())
		currentMainDialog.Hide()
		currentMainDialog = d
		d.Refresh()
	...
	collectSoulButton := widget.NewButton("Drain Soul", func() {
		...
		d := dialog.NewCustomWithoutButtons("Drain Specky for Soul", collectSoulConfirmDialog, mainWindow)
		d.Resize(mainWindowLyt.Selected().Content.Size())
		currentMainDialog.Hide()
		currentMainDialog = d
		d.Refresh()
	...
	registerNameButton := widget.NewButton(nameButtonLabel, func() {
		...
		if len(nameTaken) > 15 {
			dialog.ShowInformation("Name already forged", "Apologies, Souldier.\n\tThe name you seek is already forged by another. Choose wisely, for each name is as unique as Beskar steel. This is the way.", mainWindow)
			...
			d := dialog.NewCustomWithoutButtons("Forge a name with Specky", collectSoulConfirmDialog, mainWindow)
			d.Resize(mainWindowLyt.Selected().Content.Size())
			currentMainDialog.Hide()
			currentMainDialog = d
			d.Refresh()
			...
			registerNameDiaButtonContainer.Objects = []fyne.CanvasObject{registerNameDiaButtons}
			collectSoulConfirmDialog := container.NewBorder(nil, registerNameDiaButtonContainer, nil, nil, container.NewVBox(registerNameConfirmLabel, registerNameInfoLabel))
			d := dialog.NewCustomWithoutButtons("Reforge a name with Specky", collectSoulConfirmDialog, mainWindow)
			d.Resize(mainWindowLyt.Selected().Content.Size())
			currentMainDialog.Hide()
			currentMainDialog = d
			d.Refresh()
	...
	smRwrdButton := widget.NewButton("Honor the Masters", func() {
		...
		d := dialog.NewCustomWithoutButtons("Distribute Master's Bounty", SmRwrdConfirmDialog, mainWindow)
		d.Resize(mainWindowLyt.Selected().Content.Size())
		currentMainDialog = d
		d.Refresh()
		d.Show()
	...
	crwnRwrdButton := widget.NewButton("Forge the Crowns", func() {
		...
		confirmButton := widget.NewButton("This is the way", func() {
			keyPair, err := cryptography.FromWIF(creds.Wallets[creds.LastSelectedWallet].WIF)
			if err != nil {
				dialog.ShowError(fmt.Errorf("invalid WIF: %v", err), mainWindow)
				return
			}
		...
		d := dialog.NewCustomWithoutButtons("Forge Crowns", crwnRwrdConfirmDialog, mainWindow)
		d.Resize(mainWindowLyt.Selected().Content.Size())
		currentMainDialog = d
		d.Refresh()
		d.Show()
	...
	stakeUnstakeBttn := widget.NewButton("Drain/Power Up Specky", func() {
		currentMainDialog = dialog.NewCustomWithoutButtons("Drain/Power Up Specky", stakeContainer, mainWindow)
	...
	registerNameMainButton := widget.NewButton(nameButtonLabel, func() {
		currentMainDialog = dialog.NewCustomWithoutButtons(nameButtonLabel, registerNameContainer, mainWindow)
		currentMainDialog.Resize(fyne.NewSize(600, 340))
		currentMainDialog.Show()
	})
	...
	if core.LatestAccountData.StakedBalances.Amount.Cmp(core.SoulMasterThreshold) >= 0 { // if address have soulmaster show this
		...
		hodlTabScroll.Content = container.NewVBox(
			soulMasterMessage,
			stakedBalancesLabel,
	...
	headerHeight := hodlTabHeader.Size().Height
	hodlTabScroll.SetMinSize(fyne.NewSize(mainWindowLyt.Size().Width, mainWindowLyt.Selected().Content.MinSize().Height-headerHeight))
	hodlTab.Content = container.NewBorder(hodlTabHeader, nil, nil, nil, hodlTabScroll)
	hodlTab.Content.Refresh()
