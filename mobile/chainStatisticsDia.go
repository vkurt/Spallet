...
func buildAndShowChainStatistics() {
	...
	chnStcsCntnt := container.NewScroll(statistics)
	chnStcsLyt := container.NewBorder(nil, bckBttn, nil, nil, chnStcsCntnt)
	currentMainDialog = dialog.NewCustomWithoutButtons(fmt.Sprintf("Chain Statistics For %v", core.UserSettings.NetworkName), chnStcsLyt, mainWindow)
	currentMainDialog.Resize(fyne.NewSize(mainWindow.Canvas().Size().Width-10, mainWindow.Canvas().Size().Height-50))
	closeUpdatingDialog()
